...
from .utilities.gemini_cli_message_transform import GeminiCliMessageTransformMixin
...
class GeminiCliProvider(
    ...
    GeminiToolHandler,
    GeminiCliMessageTransformMixin,
    ...
    def _parse_quota_response(self, data: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any]]]:
        return self._parse_remaining_fraction_response(
            data,
            container_key="buckets",
            entries=lambda container: (
                (b.get("modelId"), b, b) for b in container
            ),
            extra_fields=lambda _name, entry: {
                "model_id": entry.get("modelId"),
                "remaining_amount": entry.get("remainingAmount"),
    ...
    # NOTE: Message transform methods are inherited from GeminiCliMessageTransformMixin
