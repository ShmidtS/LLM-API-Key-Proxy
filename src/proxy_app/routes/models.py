...
def _cached_models_response() -> Response:
    return Response(
        content=_models_cache["bytes"],
        media_type="application/json",
        headers={"Cache-Control": f"max-age={MODEL_CACHE_MAX_AGE}"},
    )


def get_model_info_service(request: Request) -> Any:
...
async def list_models(
    ...
    enriched: bool = True,
    model_info_service: Any = Depends(get_model_info_service),
) -> Response:
    """
    ...
    if enriched and model_info_service and model_info_service.is_ready:
        ...
        async with _models_cache_lock:
            ...
            _models_cache["expires"] = now + MODEL_CACHE_MAX_AGE
        return _cached_models_response()
